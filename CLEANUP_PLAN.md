...
- [ ] Убрать неиспользуемые `companies` / `agent_enabled` из шаблона или показать статус AI в UI
- [ ] Пометить `/diagnose/reports` как dev-only
- [ ] Обновить `README.md`, убрать устаревшее (`Flask`, загрузка шаблонов в UI, Qwen 7b)
- [ ] `companies.py` — единственный источник списка компаний (не `docx_processing.py`)
