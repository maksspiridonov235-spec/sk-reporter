...
# Два «рывка» границы столбца в Word ≈ 0,5 см (567 twips/см → 141×2 dxa).
REPORT_NUMBER_OBJECT_BORDER_NUDGE_DXA = 282
...
# False — apply_layout не меняет документ (функция и CLI остаются).
LAYOUT_PIPELINE_ENABLED = False
...
def _is_engineer_6col_shell(cols: list[str]) -> bool:
    """6 колонок, совпадают крайние — col 1–2 можно подправлять."""
    base = ENGINEER_GRID_COLS_6
    return len(cols) == 6 and cols[0] == base[0] and cols[3:] == base[3:]


def nudge_report_number_object_border(doc) -> bool:
    """
    Сдвигает границу между № отчёта (col 1) и «Объект» (col 2) влево.
    Целевые col 1–2 из engineer_grid_after_border_nudge(); крайние колонки не трогаем.
    """
    ...
    if not current or not _is_engineer_6col_shell(current):
        return False
    ...
    apply_table_grid_cols(table, new_cols)
    return True
