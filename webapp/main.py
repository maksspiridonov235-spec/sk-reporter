...
from report_date import apply_report_date
...
@app.post("/apply-date")
async def apply_date_endpoint(body: ApplyDateBody | None = None):
    body = body or ApplyDateBody()
    if body.date:
        try:
            datetime.strptime(body.date, "%Y-%m-%d")
        except ValueError:
            raise HTTPException(status_code=400, detail="date должна быть YYYY-MM-DD")
        result = apply_report_date(
            str(UPLOAD_DIR),
            str(TEMPLATES_DIR),
            iso_date=body.date,
        )
    elif body.mode in ("today", "yesterday"):
        result = apply_report_date(
            str(UPLOAD_DIR),
            str(TEMPLATES_DIR),
        ...
        result = apply_report_date(str(UPLOAD_DIR), str(TEMPLATES_DIR), mode="today")
    return result
