...
- **Что сделано:** вкладка **Проекты** — карточки в PostgreSQL (ВОР + ТЛ), UI как ОТКК («Открыть»); пилот `SUP-PDR-ENC-001-DD-ST01-EV_00.1`.
- **Что сделано:** **ВОР пилота в БД из эталона** `sk_reporter/project_data/sup_pdr_enc_00_1.json` (как ОТКК) — `.doc` в gitignore, на RelaxDev без LibreOffice; `seed_sup_pdr_pilot(overwrite=True)` при старте.
...
**Статус планирования:** Сотрудники, Подрядчики, Проекты и ОТКК — PostgreSQL (RelaxDev); пилот подрядчика — **Евракор**  
