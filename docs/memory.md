...
- **Что сделано:** удалены `engineer/profiles/*.yaml` — инженер ФИО берёт ФИО и должность только из PostgreSQL (`person_id` в URL `/engineer/{person_id}`); шаблон docx — общий `data/engineer/report_template.docx`.
...
**Статус планирования:** Сотрудники и ОТКК — PostgreSQL (RelaxDev); ОТКК — эталон в репо + seed, правки вручную таблица за таблицей; Проекты — пока файлы на диске  
