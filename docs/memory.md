...
- **Что сделано:** каталог **ОТКК** — PostgreSQL (`otkk_cards`), импорт manifest.yaml / скан `data/tk/`, UI `/planning/otkk`; сами .doc остаются на диске сервера.
- **Что сделано:** **ОТКК в БД — структура карты** (пилот `otkk-1`): парсер `.doc/.docx` → JSON; загрузка через UI; просмотр «Открыть». **Только PostgreSQL** — без manifest/диска в UI; пустые записи каталога удаляются при старте; .doc на сервер не копируется.
- **Что сделано:** **ОТКК-2** — эталон `otkk2_data.py` и сид в БД: п.4 = четыре кода СП через запятую; служебные поля Word `HYPERLINK "kodeks://..."` убираются (`sk_reporter/otkk_text.py`), не показывать в UI.
- **Что дальше:** импорт ОТКК-2…41 по одной после проверки ОТКК-1 на RelaxDev; Проекты в PostgreSQL.
...
**Статус планирования:** Сотрудники и ОТКК — PostgreSQL (RelaxDev); ОТКК — структурированный `content` (пилот otkk-1); Проекты — пока файлы на диске  
...
**Код:** эталон `otkk{N}_data.py` — `scripts/build_otkk_data.py` из `data/tk/*.doc`; в БД пишется через `sanitize_otkk_rows()` в `otkk_db.py`. На RelaxDev после `git pull` — **перезапуск** сервера (автосид `seed_otkk2(overwrite=True)`).
