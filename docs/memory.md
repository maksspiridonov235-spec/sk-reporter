...
**Дата:** 2026-06-04  
**Фокус сессии:** сайдбар. **Руководитель (2026-06-04):** список под датой; смена при **«Подготовить»** (`leader_switch.py`, SSE) — без Ollama.
...
```
загрузка → дата + руководитель (список) → Проверить и исправить
→ Подготовить → Сформировать → …
```

**Подготовить** = смена руководителя (по списку) → макросы в загрузках → дата в болванках.
...
### Руководитель в отчёте (список под датой)

| UI | Действие |
|----|----------|
| `#leaderSelect` под `#macroReportDate` | выбор Аниськов / Манджиев (И.О.); выбор в `localStorage` |

Смена подписей **не по клику в списке**, а при **«Подготовить»** (первый шаг цепочки): для всех `.docx` в `UPLOAD_DIR` — rule-based замена **4 ячеек** (шапка: должность + ФИО; подвал: должность + ФИО). Без Ollama.
