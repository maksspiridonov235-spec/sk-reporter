...
- **Что сделано:** **ОТКК в БД — структура карты** (пилот `otkk-1`): парсер `.doc/.docx` → JSON (`rows` label/value, `body` с абзацами/пунктами, нормативка, подпись); колонки `code`, `title`, `content` (JSONB); загрузка через UI «Загрузить карту» и `scripts/import_otkk_doc.py` (полная перезапись); просмотр структуры в диалоге «Открыть».
