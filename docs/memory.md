...
- **Что сделано:** справочник **Сотрудники** — PostgreSQL при `DATABASE_URL` (таблица `personnel`, `sk_reporter/db/`, импорт yaml/xlsx, UI на `/planning/personnel`, CLI `scripts/import_personnel_db.py`); без `DATABASE_URL` — как раньше `data/personnel/personnel.yaml`.
...
**Статус планирования:** Сотрудники → PostgreSQL (v1, при `DATABASE_URL`); Проекты и ОТКК — пока файлы на диске, миграция в очереди  
