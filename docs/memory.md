...
| 1 | **Сетка как в Excel** (строки = люди, столбцы = дни) | ✅ просмотр (2026-06-07) |
| 2 | **Редактирование ячеек** на сайте + сохранение в yaml | ✅ (2026-06-07) |
| 3 | **Импорт из xlsx** / **экспорт в xlsx** (синхронизация) | 🔲 **следующий код** |
...
**Этап 2 (2026-06-07):** клик по ячейке — цикл · → 1 → 0.5 → ·; клавиши `1` / `.` / `Del`. POST `/api/luvr/mark` → `update_luvr_mark()` → запись в `luvr.yaml`, пересчёт Σ. xlsx пока **не** обновляется (этап 3).
