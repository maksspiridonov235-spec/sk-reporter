...
| В браузере «Внутренняя ошибка сервера» после `git pull` | **Перезапустить** uvicorn (остановить терминал в VS Code / Ctrl+C и снова `python -m uvicorn main:app ...` из `webapp/`). Без `--reload` код в памяти не обновляется. При старте: `[INFO] UI templates: ...\webapp\templates`. Проверка: http://localhost:8000/health — `"ok": true`, `"git_head"` совпадает с последним коммитом |
