...
Обновление кода и перезапуск — на стороне RelaxDev (деплой из `main`, push в ветку → автосборка).

**Расстановка (`/deployment`):** в репозитории есть `Dockerfile` с LibreOffice — платформа собирает образ при деплое. Проверка после деплоя: `/health` → `"libreoffice": {"available": true, ...}`.
