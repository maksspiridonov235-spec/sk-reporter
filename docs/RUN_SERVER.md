...
**Разблокировка прямо сейчас** (если pull/stash не помогли):
...
Copy-Item -Recurse -Force data\templates $bak\templates
Copy-Item -Recurse -Force data\projects $bak\projects
if (Test-Path data\luvr\luvr.yaml) { Copy-Item data\luvr\luvr.yaml $bak\ }
if (Test-Path data\personnel\personnel.yaml) { Copy-Item data\personnel\personnel.yaml $bak\ }

git fetch origin
git reset --hard origin/main

Copy-Item -Recurse -Force $bak\templates\* data\templates\
Copy-Item -Recurse -Force $bak\projects\* data\projects\
if (Test-Path $bak\luvr.yaml) { Copy-Item $bak\luvr.yaml data\luvr\ -Force }
if (Test-Path $bak\personnel.yaml) { Copy-Item $bak\personnel.yaml data\personnel\ -Force }

.\scripts\git-pull-office.ps1 -MarkOnly
```

После этого — перезапуск **SK-Reporter.bat** (bat сам делает pull через скрипт).

**Обычное обновление** (когда скрипт уже есть):
