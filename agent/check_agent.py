...
SYSTEM_PROMPT = """Ты — инспектор строительного контроля, проверяешь качество заполнения ежедневных отчётов.

Правила проверки:
1. ОБЪЕМЫ: накопительный объем НЕ МОЖЕТ быть больше проектного объема
2. НУЛЕВЫЕ РАБОТЫ: если суточный объем = 0, эту работу нужно удалить из отчета
3. ОПИСАНИЯ: описание работы должно точно соответствовать типу выполняемой работы

Верни JSON строго в таком формате:
{
  "errors": [
    {"type": "volume", "work_id": 1, "message": "накопительный объем превышает проектный"},
    {"type": "zero_volume", "work_id": 2, "message": "суточный объем = 0, работу нужно удалить"},
    {"type": "description", "work_id": 3, "message": "описание не соответствует типу работы"}
  ],
  "recommendations": [
    "рекомендация 1",
    "рекомендация 2"
  ],
  "summary": "одна фраза — общее состояние отчёта"
}
...
def check_report(filepath: str) -> dict:
    ...
    user_prompt = f"""Проверь следующие работы из отчета:

{works_text}

Проверь:
1. Не превышает ли накопительный объем проектный объем для каждой работы
2. Какие работы имеют суточный объем = 0 (их нужно удалить)
3. Соответствуют ли описания работ типам выполняемых работ (это требует анализа названия и описания работы)

Верни JSON с найденными ошибками и рекомендациями."""
    ...
    try:
        ...
        result["_source_file"] = report_data.get("_source_file")
