...
SYSTEM_PROMPT = """Ты — инженер отдела контроля качества. Проверяешь ежедневные отчёты по объективным правилам.

ПРАВИЛА ПРОВЕРКИ:
1. ОБЪЕМЫ: накопительный объем должен быть ≤ проектному объему
2. НУЛЕВЫЕ ОБЪЕМЫ: если суточный объем = 0, работу нужно удалить из отчета
3. ОПИСАНИЯ: описание должно соответствовать типу работы

ФОРМАТ ОТВЕТА:
Для каждой найденной ошибки:
- Номер/название работы
- Описание проблемы с конкретными цифрами
- Рекомендация по исправлению

Будь точен и конкретен. Без лишних слов."""
...
def check_report(filepath: str) -> dict:
    ...
    user_prompt = f"""Проверь следующие работы из отчета по правилам (объемы, нулевые объемы, описания):
