#!/usr/bin/env python3
"""Залить эталон ОТКК-2 (6 пунктов) в PostgreSQL."""
...
from sk_reporter.otkk_db import purge_empty_otkk_cards, seed_otkk2


def main() -> int:
    parser = argparse.ArgumentParser(description="Эталон ОТКК-2 в PostgreSQL")
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Перезаписать, если otkk-2 уже есть",
    )
    ...
    try:
        result = seed_otkk2(overwrite=args.overwrite)
